...
export async function getAdminAnalytics(filters: { from?: string; to?: string; city?: string }) {
  ...
  const totals = await dbQuery<{ redemptions: string; unique_customers: string }>(
    `
      SELECT COUNT(*)::text AS redemptions, COUNT(DISTINCT user_id)::text AS unique_customers
      FROM redemptions
      ${clause}
    `,
    values,
  ...
  const cardUsage = await dbQuery<{ card_id: string; card_name: string; redemptions: string }>(
    `
      SELECT c.id AS card_id, c.name AS card_name, COUNT(r.id)::text AS redemptions
      FROM redemptions r
  ...
  return {
    ...
    usageByVendor: vendorUsage.map((item) => ({
      vendorId: item.vendor_id,
      vendorName: item.vendor_name,
      redemptions: Number(item.redemptions),
    })),
    usageByCard: cardUsage.map((item) => ({
      cardId: item.card_id,
...
export async function getVendorAnalytics(vendorId: string) {
  ...
  const cards = await dbQuery<{ card_id: string; card_name: string; redemptions: string; unique_customers: string }>(
    `
      SELECT c.id AS card_id,
             ...
             COUNT(DISTINCT r.user_id)::text AS unique_customers
      FROM redemptions r
      JOIN cards c ON c.id = r.card_id
      WHERE r.vendor_id = $1
      GROUP BY c.id, c.name
  ...
  return {
    ...
    byCard: cards.map((item) => ({
      cardId: item.card_id,
      cardName: item.card_name,
      redemptions: Number(item.redemptions),
      uniqueCustomers: Number(item.unique_customers),
    })),
  };
}
